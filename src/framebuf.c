...
SDL_Renderer *framebuf_init(Framebuffer *fb, State *state, rgba color) {
  ...
  fb->screen = SDL_GetWindowSurface(state->window);
  // check it didnt fail
  if (fb->screen == NULL) {
    // uh oh
  ...
  fb->format = SDL_GetPixelFormatDetails(
      fb->screen->format); // clang is crying because ive set a non constant
                           // value to a constant value - cry about it
...
void framebuf_column_optimised(Framebuffer *fb, int *top, int *bottom,
  ...
  // drawing a vertical column is inefficent, so this draws the walls in a
  // horizontal way - confusing but it should give a noticable frame increase
  for (int y = 0; y < WINDOW_HEIGHT; y++) {
    uint32_t *ptr = fb->pixels + y * WINDOW_WIDTH;
